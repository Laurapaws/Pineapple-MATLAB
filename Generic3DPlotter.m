...
ylabel('Braid Length (cm)');                     % Add x y and z labels
zlabel('Impedance ({\Omega})');

            fSlice = 868;                         % Selected frequency
            lSlice = 200;                        % Distance through independant variable
...
mainTitle = 'Impedance of 18x1mm Tinned Copper Braid';
title({
    [mainTitle] 
...
set(gca,'YScale','log');
c1.LineWidth = 1.7;
title({ ["Impedance vs Frequency at " + lSlice + "cm"]});
xlabel('Frequency (MHz)');               
...
set(gca,'XScale','log');
c2.LineWidth = 1.7;
title({ ["Impedance vs Length at " + fSlice + "MHz"]});
